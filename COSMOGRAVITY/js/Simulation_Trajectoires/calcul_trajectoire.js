...
//  Les différents "temps" et autres valeurs à afficher
	...
	if (element2.value != "mobile"){ // observateur
		if(mobile.r_part_obs >= 1.00001*rs){
			mobile.temps_observateur += mobile.dtau;  //console.log("1180",mobile.dtau);
		...
		if (mobile.r_part>0){	
			//mobile.temps_particule+=mobile.dtau; //console.log("1722",mobile.dtau);
...
function enregistrer() {
	if (document.getElementById('trace_present').value === "1") {
		...
		if (sessionStorage.getItem("LANGUE") === "fr") {
			var nomFichier = prompt("Veuillez entrer le nom du fichier désiré :", "Trajectoire_bary_bary");
		}
		if (sessionStorage.getItem("LANGUE") === "en") {
			var nomFichier = prompt("Please enter the desired file name :", "Trajectory_bary_bary");
		}
