...
/*	if(amax<1){
	...
	a2=a2;  
			...
			age_univ_sec = Number(age_sec) + temps_restant;   console.log("277 ageunivsec modele ",age_univ_sec,modele);
		}
		...
		console.log("282 age_univ_Ga",age_univ_Ga);
		...
		console.log("285 age_univ_sec",age_univ_sec);
		}
...
// Tracer graphique
function graphique_creation_noir(boolBigRip,modele) { //S Ajout d'un booléen pour manifester l'existence d'un big rip ou non
	...
	if((boolBigRip)){//S	
		dureeUniv = document.getElementById("resultat_dureeuniv").innerHTML  //S
		//S Méthode de singe qui fonctionne
		dureeUnivJPC = dureeUniv.split(' ') //S
		dureeUnivJPC = parseFloat(dureeUnivJPC[0]) //S
		...
		showlegend: false	   
			...
			dureeUniv = dureeUniv.split(' ') //S
			dureeUniv = parseFloat(dureeUniv[0])
		}//S 
	...
	if (window.innerWidth > 960) {
		annots = [{//Ici définit des infos relatif aux "entrées"(légende graph), dont la position.
				...
				xref: 'paper',
				xanchor: 'center',
				y: 0.99, //S au-dessus de 1, il semble sortir du graph, on décale donc x pour ne plus avoir de conflit avec l'axe des a( q.14)
				yref: 'paper',
				yanchor: 'bottom',
			...
			},
			{
				"xref": "paper",
				"yref": "paper",
				"text": "<b>" + texte.calculs_univers.sorties + "</b><br>Ω<sub>r0<\/sub>= " + Or + "<br>Ω<sub>k0</sub>= " + omegak0 + "<br>t<sub>BB</sub>= " + age_afficher + " Ga" + "<br>t<sub>Univ</sub> = " + dureeUniv + " Ga"				,
				"y": 1.026,
				"x": 1,
				xanchor: 'center',
				yanchor: 'bottom',
