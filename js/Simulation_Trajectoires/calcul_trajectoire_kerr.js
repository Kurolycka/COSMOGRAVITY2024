...
//khaled a rajouté ça pour les couleurs du graphe
...
function animate() {
	...
	if (r0 != 0.0) {
		...
		if (element2.value != "mobile"){
			...
			if(r_part_obs<rhp ) { r_part_obs=rhp;} //khaled a changé ça pour le bug de R qui n'arrive pas exactement à Rh+

			A_part_obs = val_obs[1];
			
			if( r_part_obs<rhp ) { vr_3_obs=0;}
			...
			posX2 = scale_factor * Math.sqrt(r_part_obs**2+a**2)* (Math.sin(teta_spherique) *Math.cos(phi_obs) / rmax) + (canvas.width / 2);//khaled
			posY2 = scale_factor * Math.sqrt(r_part_obs**2+a**2) * (Math.sin(teta_spherique) *Math.sin(phi_obs) / rmax) + (canvas.height / 2);
