...
var c, G, h, k, t0, h0, omegam0, omegalambda0, omegak0, Or;
var texte, nbrjours, H0parsec, H0enannee, H0engannee;
var age_sec, age, age_afficher, age_sec_afficher;
...
// initialise toutes les variables en fonction de la valeur des omega
// calcul l'age de l'univers, si big bang, si big crunch,... et envoie tout sur la page html simulation
function parametres() {
  ...
  omegalambda0 = ajustePrecision_Scientifique(omegalambda0);
...
// trace la fonction pour afficher la courbe dans simulation
function calcul() {
  texte=o_recupereJson()
  ...
  //On récupère les bornes pour le graphe
  var a_min = Number(document.getElementById("a_min").value);
  var a_max = Number(document.getElementById("a_max").value);

  //calcul du pas
  var zmax, agefinal, agedebut, exposant, zmin, pas, temps;
  zmax= (1-a_max)/a_max;
  ...
  while (fy < a_max) {
    ...
    // J'ai modifie le pas avec un facteur *100 car ça ramait et consommait de la ram à l'infini au bout de 3/4 tracés !
    temps = temps + pas;
  }
  ...
  omegalambda0 = ajusteEnEntier(omegalambda0);
  omegam0 = ajusteEnEntier(omegam0);
  omegak0 = ajusteEnEntier(omegak0);
  h0 = ajustePrecisionCentieme(h0);

  var title, leg_gauche, leg_droite, tracer1, annots;
  title=texte.calculs_univers.titre;
  leg_gauche="<b>" +texte.calculs_univers.entrees+ "</b><br> <br>Ω<sub>m0</sub>= " + omegam0 + " ; Ω<sub>Λ0</sub>= " + omegalambda0 + "<br>Ω<sub>r0</sub>= " + Or + " ; Ω<sub>k0</sub>= " + omegak0 + "<br>" + " H<sub>0</sub>= " + h0 + " km.s<sup>-1</sup>.Mpc<sup>-1</sup><br>";
  leg_droite="<b>" +texte.calculs_univers.sorties+ "</b><br>T<sub>0</sub>= " + t0 + " K<br>" + "t<sub>BB</sub>= " + age_afficher + " Ga<br>Duree= ∞ Ga";
