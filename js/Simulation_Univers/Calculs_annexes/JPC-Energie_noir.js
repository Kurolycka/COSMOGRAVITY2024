...
//z2
  ...
  dl2_lum=dl2_lum.toExponential(4);
  	...
  	mu_1 = (-5 + 5* Math.log10(dl_pc)).toExponential(4)
	mu_2 = (-5 + 5* Math.log10(dl2_pc)).toExponential(4)
  ...
  if (dz1 != 0) {
    dz1 = dz1.toExponential(4);
  } 
  if (dz2 != 0) {
    dz2 = dz2.toExponential(4);
  }
	...
	if(isNaN(age_ans)) {modele=1;}
		...
		if(d_checkbox.checked) {
			...
			var abscissa_d = linear_scale(zmin, zmax, pas);  
					...
					if (sessionStorage.getItem("LANGUE") == "fr") {frtitle="al";}else{frtitle="ly";}
