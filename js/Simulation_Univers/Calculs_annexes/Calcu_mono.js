...
function avertissement() {
  ...
  if (z1 <= -1) {
    messagebox(texte.page_univers_calculs.erreur,texte.page_univers_calculs.message_z1_incorrect);
    z1 = NaN;
  }
  if (z2 <= -1) {
    messagebox(texte.page_univers_calculs.erreur,texte.page_univers_calculs.message_z2_incorrect);
    z2 = NaN;
  }
  // nécessaire car des bugs se créent au delà
  if (z1 > 1e77 || z2 > 1e77) {
    messagebox(texte.page_univers_calculs.erreur,texte.page_univers_calculs.message_valeur_critique_1e77);
    z1 = NaN;
    z2 = NaN;
  }
}
...
function calcu(path) {
  ...
  if (omegalambda0 != 1) {
    ...
    agebetween_sec = (1.0 / H0parsec) * agebetween / (1. / H0enannee);
  }
