...
//Simpson utilisé pour le calcul du temps dans lenergie sombre ! 

function simpson_simple_degre2_EN(fonction,bornInf, omegam0, omegaD0, Or,w0,w1){
  var Sum=0.0;
  var n=Math.pow(10,5);
  for(var i=1;i<=7;i+=1){
    if(bornInf<100 && i==1){
    ...
    var S=0.0;
    var S_0=fonction(bornI,omegam0,omegaD0,Or,w0,w1)+fonction(bornS,omegam0,omegaD0,Or,w0,w1);
    var S_1=0.0;
    var S_2=0.0;
    var h=(bornS-bornI)/n;
    for(var o=1;o<n-1;o+=1){
      var x1=bornI+o*h;
      if ((o%2) == 0){
        S_2+=fonction(x1,omegam0,omegaD0,Or,w0,w1);
        }
      else{
        S_1+=fonction(x1,omegam0,omegaD0,Or,w0,w1);
      }
    }
    S=(h/3.0)*( S_0 + 4.0*S_1 + 2.0*S_2);
    Sum+=S;
