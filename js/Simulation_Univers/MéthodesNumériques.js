...
function integraleTrapezes(x, y, debut, fin) {
    if (x.length !== y.length || x.length < 2 || debut >= fin) {
        return null; // Vérification des longueurs des tableaux et bornes valides
    }
    ...
    // Trouver l'indice de départ correspondant à la borne de début
    while (startIndex < x.length && x[startIndex] < debut) {
        startIndex++;
    }
    
    // Trouver l'indice de fin correspondant à la borne de fin
    while (endIndex >= 0 && x[endIndex] > fin) {
        endIndex--;
    }
    
    for (let i = startIndex; i < endIndex; i++) {
        let base = x[i + 1] - x[i];
        let hauteurMoyenne = (y[i] + y[i + 1]) / 2;
        integrale += base * hauteurMoyenne;
    }
    return integrale;
}