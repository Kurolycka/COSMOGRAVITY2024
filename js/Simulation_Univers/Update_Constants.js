...
function update_rho(path){
	...
	rho_m = omegam0*3 * Math.pow(h0*1e3,2)/ Math.pow(3.085677581*1e22,2) / (8 * pi() * G); // 1parsec=3.085677581*1e16 mètres
...
function update_omega_mono(){
	...
	document.getElementById("Omcalc").title="djhehgfuyie"
}
...
function update_omegar0_calc_noir(){
	...
	document.getElementById("Orr").innerHTML = Or;
	update_omegak0_calc_noir();update_rho(1);
}
