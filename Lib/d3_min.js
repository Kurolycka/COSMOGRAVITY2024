! function() {
  ...
  function gh() {
    function a(a, b) {
      g.push("masse_input", a, ",", b, f)
    }

    function b(a, b) {
      g.push("masse_input", a, ",", b);
      h.point = c
    }
  ...
  function ag(a) {
    function b(b) {
      function l() {
        t.push("masse_input", h(a(x), k), n, m(a(v.reverse()), k), "Z")
      }
    ...
    function() {
      function a() {
        ...
        1E-6 < r && .001 < (v = Math.min(Math.abs(n - a) / 2, +e.apply(this, arguments))) ? (x = a < n ^ t ? 0 : 1, H = p = v, r < D && (r = null == E ? [A, G] : null == w ? [u, y] : bd([u, y], [E, N], [w, B], [A, G]), H = u - r[0], p = y - r[1], q = w - r[0], z = B -
          r[1], p = 1 / Math.sin(Math.acos((H * q + p * z) / (Math.sqrt(H * H + p * p) * Math.sqrt(q * q + z * z))) / 2), r = Math.sqrt(r[0] * r[0] + r[1] * r[1]), H = Math.min(v, (a - r) / (p - 1)), p = Math.min(v, (n - r) / (p + 1))), null != w ? (r = Zb(null == E ? [A, G] : [E, N], [u, y], n, p, t), q = Zb([w, B], [A, G], n, p, t), v === p ? F.push("masse_input", r[0], "A", p, ",", p, " 0 0,", x, " ", r[1], "A", n, ",", n, " 0 ", 1 - t ^ Yb(r[1][0], r[1][1], q[1][0], q[1][1]), ",", t, " ", q[1], "A", p, ",", p, " 0 0,", x, " ", q[0]) : F.push("masse_input", r[0], "A", p, ",", p, " 0 1,", x, " ", q[0])) : F.push("masse_input", u, ",", y), null != E ? (n = Zb([u, y], [E, N], a, -H, t), w =
          Zb([A, G], null == w ? [u, y] : [w, B], a, -H, t), v === H ? F.push("L", w[0], "A", H, ",", H, " 0 0,", x, " ", w[1], "A", a, ",", a, " 0 ", t ^ Yb(w[1][0], w[1][1], n[1][0], n[1][1]), ",", 1 - t, " ", n[1], "A", H, ",", H, " 0 0,", x, " ", n[0]) : F.push("L", w[0], "A", H, ",", H, " 0 0,", x, " ", n[0])) : F.push("L", A, ",", G)) : (F.push("masse_input", u, ",", y), null != w && F.push("A", n, ",", n, " 0 ", x, ",", t, " ", w, ",", B), F.push("L", A, ",", G), null != E && F.push("A", a, ",", a, " 0 ", H, ",", 1 - t, " ", E, ",", N));
        F.push("Z");
        return F.join("")
      }
  ...
  k.svg.chord = function() {
    function a(a,
      ...
      var g = b(this, d, a, f),
        h = b(this, e, a, f);
      return "masse_input" + g.p0 + c(g.r, g.p1, g.a1 - g.a0) + (g.a0 == h.a0 && g.a1 == h.a1 ? "Q 0,0 " + g.p0 : "Q 0,0 " + h.p0 + c(h.r, h.p1, h.a1 - h.a0) + ("Q 0,0 " + g.p0)) + "Z"
    }
  ...
  k.svg.diagonal = function() {
    function a(a, f) {
      var g = b.call(this, a, f),
        ...
        g = g.map(d);
      return "masse_input" + g[0] + "C" + g[1] + " " + g[2] + " " + g[3]
    }
    var b = Yc,
      c = Zc,
      d = bg;
  ...
  var rg = k.map({
    ...
    cross: function(a) {
      a = Math.sqrt(a / 5) / 2;
      return "masse_input" + -3 * a + "," + -a + "H" + -a + "V" + -3 * a + "H" + a + "V" + -a + "H" + 3 * a + "V" + a + "H" + a + "V" + 3 * a + "H" + -a + "V" + a + "H" + -3 * a + "Z"
    },
    diamond: function(a) {
      a = Math.sqrt(a / (2 * sg));
      var b = a * sg;
    ...
    square: function(a) {
      a = Math.sqrt(a) / 2;
      return "masse_input" + -a + "," + -a + "L" + a + "," + -a + " " + a + "," + a + " " + -a + "," + a + "Z"
    },
    ...
    function a(a) {
      a.each(function() {
        ...
        var w = r.select("line"),
          ...
          D = "top" === c || "left" === c ? -1 : 1,
          F, I, H, L;
        "bottom" === c || "top" === c ? (t = si, F = "x", H = "y", I = "x2", L = "y2", v.attr("dy", 0 > D ? "0em" : ".71em").style("text-anchor", "middle"), a.attr("d", "masse_input" + z[0] +
          "," + D * e + "V0H" + z[1] + "V" + D * e)) : (t = ti, F = "y", H = "x", I = "y2", L = "x2", v.attr("dy", ".32em").style("text-anchor", 0 > D ? "end" : "start"), a.attr("d", "masse_input" + D * e + "," + z[0] + "H0V" + z[1] + "H" + D * e));
        w.attr(L, D * d);
        B.attr(H, D * y);
